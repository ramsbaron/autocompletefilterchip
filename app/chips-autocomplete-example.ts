...
export class ChipsAutocompleteExample {
  ...
  constructor() {
    this.filteredItems = this.drgCtrl.valueChanges.pipe(
        startWith(''),
        map((item: string | null) => item ? this.filter(item) : this.allItems.slice()));
  }
  ...
  filter(name: string) {
    return this.allItems.filter(item => item.toLowerCase().includes(name.toLowerCase()));
  }
  ...
  selected(event: MatAutocompleteSelectedEvent): void {
    ...
    const selectedIndex = this.allItems.indexOf(selectedItem);
    this.allItems.splice(selectedIndex,1);
